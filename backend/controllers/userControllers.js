...
const loginController = async (req, res) => {
  try {
    ...
    const jwtSecret = process.env.JWT_KEY || "change_me_to_a_strong_secret";
    if (!process.env.JWT_KEY) {
      console.warn(
        "WARNING: JWT_KEY is not set in .env. Using default secret. Set JWT_KEY to a strong value in .env when deploying."
      );
    }
...
const postCourseController = async (req, res) => {
  try {
    ...
    // Normalize file uploads and section fields to arrays
    const files = Array.isArray(req.files) ? req.files : [];
    const titles = Array.isArray(S_title)
      ? S_title
      : S_title
      ? [S_title]
      : [];
    const descriptions = Array.isArray(S_description)
      ? S_description
      : S_description
      ? [S_description]
      : [];
    ...
    const sections = titles.map((title, idx) => ({
      S_title: title,
      S_description: descriptions[idx] || "",
      S_content: files[idx] ? files[idx].filename : "",
    }));

    const price = Number(C_price) === 0 ? "free" : C_price;
    ...
    return res
      .status(201)
      .send({ success: true, message: "Course created successfully", data: course });
  } catch (error) {
...
const enrolledCourseController = async (req, res) => {
  ...
  try {
    ...
    const courseSections = Array.isArray(course.sections) ? course.sections : [];
    const course_Length = courseSections.length;
